...
考虑到读者可能不了解反向代理相关技术，我们推荐使用宝塔面板安装 Nginx。
...
如读者了解反向代理相关技术，推荐使用 1Panel 或者手动写 Nginx 配置文件进行反代，你可以将下列的反代配置直接写到配置文件上，须注意格式！
...
注意：音乐播放的歌单依赖于 [Kami 设置](#kami-设置) 中设置的网易云歌曲 ID；若没有设置，则使用默认的；同时该函数是歌单函数的依赖函数。
